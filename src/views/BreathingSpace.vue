...
const palette = computed(() => {
  ...
  const key = currentKey.value in PHASE_COLORS ? currentKey.value : "inhale";
  return PHASE_COLORS[key][scheme];
});
...
<template>
  <div :class="pageWrapClass" class="breath-page premium-bg premium-emerald">
    ...
    <section
      class="mx-auto max-w-5xl px-6 py-10 select-none"
      aria-label="Guided Breathing Session"
    >
      <!-- 顶部引导文案 -->
      <div class="text-center">
        <p class="mt-1" :class="subTextClass">
          Follow the rhythm: Inhale • Hold • Exhale
        </p>
      </div>

      <div class="mt-8 grid grid-cols-1 md:grid-cols-1 gap-6 items-center">
        <div class="flex items-center justify-center">
          ...
          >
            ...
            <svg
              ...
              class="block"
              role="img"
              :aria-label="`Breathing ring, phase ${currentPhase}`"
            >
              <defs>
                <!-- 动态渐变：跟随当前相位颜色 -->
                <linearGradient
      ...
      <div class="preset-row mt-8 flex justify-center gap-3">
        <button
          v-for="p in PRESETS"
          :key="p.seconds"
      ...
      <p class="page-tip mt-6 text-xs text-center" :class="subtleTextClass">
...
export default {
  ...
  components: {
    LegendPill: {
      props: { label: String, a: String, b: String },
      template: `
        <div class="flex items-center gap-2">
          <span class="inline-block h-3 w-6 rounded-full" :style="{ background: 'linear-gradient(90deg, ' + a + ', ' + b + ')' }"></span>
